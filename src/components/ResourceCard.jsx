...
export default function ResourceCard({ lib, categoryLabel, accent, isNew, isCopied, query, stacks = [], onCopy, onVisit }) {
  ...
  const isRedAccent = accent?.toUpperCase() === "#B00020";
  ...
  return (
    <article className="directory-resource-card" style={{ "--card-accent": cardAccent, "--card-accent-dark": isRedAccent ? "#FF4568" : undefined }}>
      ...
      <div className="directory-card-body">
        <div className="directory-card-meta"><span className="directory-card-category" style={{ "--category-accent": cardAccent, "--category-accent-dark": isRedAccent ? "#FF4568" : undefined }}>{categoryLabel}</span>{isNew && <span className="directory-new-badge">New</span>}<span className="directory-card-url">{lib.url}</span></div>
